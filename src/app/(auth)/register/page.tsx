...
import {
    Mail, Lock, Eye, EyeOff, UserPlus, Loader2, User,
    GraduationCap, BookOpen, ShieldCheck,
} from "lucide-react";
import { createClient } from "@/lib/supabase/client";

const roles = [
    { value: "student", label: "Học sinh", icon: GraduationCap, description: "Học tập & thí nghiệm" },
    { value: "teacher", label: "Giáo viên", icon: BookOpen, description: "Quản lý lớp học" },
    { value: "admin", label: "Quản trị viên", icon: ShieldCheck, description: "Quản lý hệ thống" },
];
...
export default function RegisterPage() {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        if (data.user && !data.session) {
            setSuccess("Tài khoản đã được tạo! Vui lòng kiểm tra email để xác nhận tài khoản.");
            setLoading(false);
            return;
        }
                ...
                <form onSubmit={handleRegister}>
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                {roles.map((r) => (
                                    ...
                                    >
                                        ...
                                        <span className={`text-xs font-medium ${role === r.value ? "text-primary" : "text-foreground"}`}>
                                            {r.label}
                                        </span>
                    ...
                    <CardFooter className="flex flex-col gap-4">
                        ...
                        <p className="text-center text-xs text-muted-foreground">
                            ...
                            và{" "}
                            <a href="#" className="text-primary hover:underline">Chính sách bảo mật</a> của EduLab.
                        </p>
                    </CardFooter>
                </form>
            </Card>
